...
extension SZMentionsListener {
    ...
    @discardableResult public func addMention(_ mention: CreateMention) -> Bool {
        ...
        currentMentionRange = NSRange(location: currentMentionRange.location, length: 1+mention.mentionName.utf16.count)
        ...
        delegate?.textViewDidChange!(mentionsTextView)
    ...
    fileprivate func adjust(_ textView: UITextView, range: NSRange) {
        ...
        if mentionEnabled {
            ...
            if !mentionString.isEmpty {
                DeveloperTools.print("szm :", textView.text)
                ...
                if let filterString = filterString, let cooldownTimer = cooldownTimer, !cooldownTimer.isValid  {
                    stringCurrentlyBeingFiltered = filterString
                    mentionsManager.showMentionsListWithString(filterString)
//                    DeveloperTools.print("st1: ", mentionString)
                }
...
extension SZMentionsListener: UITextViewDelegate {
    ...
    public func textView(_ textView: UITextView, shouldChangeTextIn range: NSRange,
        ...
        let parentResult = delegate?.textView?(textView, shouldChangeTextIn: range, replacementText: text)
        shouldAdjust(textView, range: range, text: text)
        return parentResult!//shouldAdjust(textView, range: range, text: text)
    }
